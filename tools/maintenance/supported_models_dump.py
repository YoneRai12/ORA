...
class StableAudio(supported_models_base.BASE):
    ...
    def get_model(self, state_dict, prefix="", device=None):
        seconds_start_sd = utils.state_dict_prefix_replace(
            state_dict, {"conditioner.conditioners.seconds_start.": ""}, filter_keys=True
        )
        seconds_total_sd = utils.state_dict_prefix_replace(
            state_dict, {"conditioner.conditioners.seconds_total.": ""}, filter_keys=True
        )
    ...
    def process_unet_state_dict(self, state_dict):
        for k in list(state_dict.keys()):
            if (
                k.endswith(".cross_attend_norm.beta") or k.endswith(".ff_norm.beta") or k.endswith(".pre_norm.beta")
...
class Flux(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        t5_detect = comfy.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(
            comfy.text_encoders.flux.FluxTokenizer, comfy.text_encoders.flux.flux_clip(**t5_detect)
        )
...
class GenmoMochi(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        t5_detect = comfy.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(
            comfy.text_encoders.genmo.MochiT5Tokenizer, comfy.text_encoders.genmo.mochi_te(**t5_detect)
        )
...
class LTXV(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        t5_detect = comfy.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(
            comfy.text_encoders.lt.LTXVT5Tokenizer, comfy.text_encoders.lt.ltxv_te(**t5_detect)
        )
...
class HunyuanVideo(supported_models_base.BASE):
    ...
    def process_unet_state_dict(self, state_dict):
        ...
        for k in list(state_dict.keys()):
            key_out = k
            key_out = key_out.replace("txt_in.t_embedder.mlp.0.", "txt_in.t_embedder.in_layer.").replace(
                "txt_in.t_embedder.mlp.2.", "txt_in.t_embedder.out_layer."
            )
            key_out = key_out.replace("txt_in.c_embedder.linear_1.", "txt_in.c_embedder.in_layer.").replace(
                "txt_in.c_embedder.linear_2.", "txt_in.c_embedder.out_layer."
            )
            ...
            key_out = key_out.replace("_attn_q_norm.weight", "_attn.norm.query_norm.scale").replace(
                "_attn_k_norm.weight", "_attn.norm.key_norm.scale"
            )
            key_out = key_out.replace(".q_norm.weight", ".norm.query_norm.scale").replace(
                ".k_norm.weight", ".norm.key_norm.scale"
            )
...
class CosmosT2V(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        t5_detect = comfy.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(
            comfy.text_encoders.cosmos.CosmosT5Tokenizer, comfy.text_encoders.cosmos.te(**t5_detect)
        )
...
class Lumina2(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        return supported_models_base.ClipTarget(
            comfy.text_encoders.lumina2.LuminaTokenizer, comfy.text_encoders.lumina2.te(**hunyuan_detect)
        )
...
class WAN21_T2V(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        return supported_models_base.ClipTarget(
            comfy.text_encoders.wan.WanT5Tokenizer, comfy.text_encoders.wan.te(**t5_detect)
        )
...
class Chroma(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        t5_detect = comfy.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(
            comfy.text_encoders.pixart_t5.PixArtTokenizer, comfy.text_encoders.pixart_t5.pixart_te(**t5_detect)
        )
...
class Omnigen2(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        return supported_models_base.ClipTarget(
            comfy.text_encoders.omnigen2.Omnigen2Tokenizer, comfy.text_encoders.omnigen2.te(**hunyuan_detect)
        )
...
class QwenImage(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict=None):
        ...
        return supported_models_base.ClipTarget(
            comfy.text_encoders.qwen_image.QwenImageTokenizer, comfy.text_encoders.qwen_image.te(**hunyuan_detect)
        )
