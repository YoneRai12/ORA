...
from src.utils.llm_client import TransientHTTPError, robust_json_request
...
async def test_robust_json_request_cancelled_error():
    ...
    with pytest.raises(asyncio.CancelledError):
        await robust_json_request(session, "GET", "http://test", _sleep_func=mock_sleep, _time_func=mock_time.monotonic)
...
async def test_robust_json_request_success():
    ...
    result = await robust_json_request(
        session, "GET", "http://test", _sleep_func=mock_sleep, _time_func=mock_time.monotonic
    )
