...
class ORACog(commands.Cog):
        ...
        try:
            ...
            if tool_name == "music_play":
                ...
                if media_cog:
                    ...
                    return f"Now playing: {query}" # Assuming query is the title or a good placeholder
                ...
                msg = f"**Now Playing:** {state['current'] or 'None'}\n**Queue:**\n"
            ...
            # Actually, standardizing:
                ...
                if hasattr(self.bot, "voice_manager"):
                    ...
                    return "VoiceManager instance not found on Bot."
            ...
            if tool_name == "google_search":
                ...
                try:
                    ...
                    vc = await self.bot.voice_manager.ensure_voice_client(message.author)
                    if vc:
                ...
                pitch = float(args.get("pitch", 1.0))
                self.bot.voice_manager.set_speed_pitch(message.guild.id, speed, pitch)
                return f"🎵 Adjusted Check: Speed={speed}, Pitch={pitch}"

            elif tool_name == "music_seek":
                ts = args.get("timestamp")
                # Parse timestamp logic duplicate from MediaCog?
                # Ideally move _parse_timestamp to utils, but simple logic here:
                seconds = 0
                try:
                    ...
                    return "Error: Invalid timestamp format (use MM:SS or seconds)."

                self.bot.voice_manager.seek_music(message.guild.id, seconds)
                return f"⏩ Seeked to {ts} ({seconds}s)."

            elif tool_name == "say":
                content = args.get("message")
                target_ch_name = args.get("channel_name")

                channel = message.channel
                if target_ch_name:
                     found = discord.utils.find(lambda c: target_ch_name.lower() in c.name.lower(), message.guild.text_channels)
                     if found: channel = found

                await channel.send(content)
                return f"Sent message to {channel.name}: {content}"

            elif tool_name == "cleanup_messages":
                count = min(100, int(args.get("count", 10)))
                deleted = await message.channel.purge(limit=count)
                return f"🗑️ Deleted {len(deleted)} messages."

            elif tool_name == "get_logs":
                lines = int(args.get("lines", 50))
                # Invoke CoreCog logic? Or just read file directly for simplicity
            ...
            elif tool_name == "dev_request":
                req = args.get("request")
                # Forward to SystemCog or Healer
                await self.bot.healer.propose_feature(feature=req, context="Natural Language Dev Request", requester=message.author)
                return f"🧬 Genetic Request Accepted: '{req}'. Evolution process initiated."
                ...
                if target == "music":
                    if hasattr(self.bot, "voice_manager"):
                        self.bot.voice_manager.set_music_volume(message.guild.id, vol_float)
                        return f"Music Volume set to {value}%."
                    return "Voice System unavailable."
                elif target == "tts":
                     if hasattr(self.bot, "voice_manager"):
                        self.bot.voice_manager.set_tts_volume(message.guild.id, vol_float)
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.guild and (self.bot.user in message.mentions or is_reply_to_me):
            ...
            # if len(clean_content) < 30 and any(k in clean_content for k in ["きて", "来て", "join"]):
            #     media_cog = self.bot.get_cog("MediaCog")
            #     if media_cog:
            #         try:
            ...
            # if len(clean_content) < 30 and any(k in clean_content for k in ["消えて", "ばいばい", "バイバイ", "帰って", "leave"]):
            #     media_cog = self.bot.get_cog("MediaCog")
            ...
            if any(k in message.content for k in ["ロール一覧", "ロール順位", "role list", "role rank"]):
    ...
    def _get_tool_schemas(self) -> list[dict]:
        ...
        return [
            ...
            },
            {
                ...
                "tags": ["music", "queue", "list", "next", "再生リスト", "キュー", "次"],
            },
            {
                "name": "music_tune",
                ...
                "tags": ["music", "tune", "speed", "pitch", "tempo", "速度", "ピッチ", "調整"],
            },
            {
                "name": "music_seek",
                ...
                "tags": ["music", "seek", "jump", "time", "シーク", "時間", "移動"],
            },
            {
                "name": "say",
                ...
                "tags": ["admin", "say", "speak", "send", "message", "話す", "送信", "メッセージ"],
            },
            {
                ...
                "tags": ["admin", "delete", "purge", "clear", "cleanup", "削除", "消去", "クリーンアップ"],
            },
            {
                "name": "get_logs",
                ...
                "tags": ["creator", "logs", "system", "debug", "ファイル", "ログ", "デバッグ"],
            },
            {
                "name": "dev_request",
                ...
                "tags": ["creator", "dev", "feature", "request", "evolution", "開発", "機能", "要望", "進化"],
            },
            {
